...
const ProductInfo = ({ product }) => {
  ...
  const isMaxReached = !isNaN(maxQty) && maxQty > 0 && quantity >= maxQty;
          ...
          {productImages.map((img, idx) => (
            <img
              ...
              className={`w-20 h-20 object-cover rounded-lg border cursor-pointer transition-transform duration-200 ${
                selectedImage === img ? "ring-2 ring-[#E07385]" : "hover:scale-105"
              }`}
            />
          ))}
        </div>
          ...
          {quantity > 0 ? (
            <div className="flex items-center border rounded-full overflow-hidden w-full sm:w-1/2">
              ...
              <button
                ...
                className={`w-1/3 py-2 text-[#E07385] font-bold transition ${
                  isMaxReached
